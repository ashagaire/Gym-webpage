...
<SectionWrapper id="project">
    ...
    >
        ...
        {#each productFetures as productFeture, index}
            <ProductCard {productFeture} {index}>
                {#if index === 0}
                    Experience a beginner exercise
                    <span class="text-indigo-400">master</span>
                    class
                {:else if index === 1}
                    Lay the foundation for
                    <span class="text-indigo-400">routine</span>
                    and consistency
                {:else}
                    Workout specific to your
                    <span class="text-indigo-400">environment</span>
                    and
                    <span class="text-indigo-400">expertise</span>
                {/if}
            </ProductCard>
        {/each}
    </div>
